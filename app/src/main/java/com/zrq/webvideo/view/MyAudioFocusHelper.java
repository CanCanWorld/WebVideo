 ...
 * 音频焦点改变监听
 */
final class MyAudioFocusHelper implements AudioManager.OnAudioFocusChangeListener {

    private final Handler mHandler = new Handler(Looper.getMainLooper());

    private final WeakReference<MyBaseVideoView> mWeakVideoView;
    ...
    MyAudioFocusHelper(@NonNull MyBaseVideoView videoView) {
    ...
    private void handleAudioFocusChange(int focusChange) {
        final MyBaseVideoView videoView = mWeakVideoView.get();
        if (videoView == null) {
            return;
        }
        switch (focusChange) {
